...
| team | [x] Team phases reach terminal states. [ ] Worker launch/model routing is deterministic. [x] Worktree cleanup is covered. | In progress | Spawn env baseline fixed; cleanup now removes safe clean worktrees even when unrelated backup blockers keep team state. Runtime-v2 monitor now persists terminal `phase-state.json` for completed/failed task sets, and Claude worker launch args collapse duplicate/conflicting model flags to one canonical `--model <value>`. Next: audit remaining model-routing gaps. |
...
- Audit remaining team model-routing gaps now that cleanup, terminal phase persistence, and explicit Claude `--model` normalization are covered.
