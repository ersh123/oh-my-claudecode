...
| team | [ ] Team phases reach terminal states. [ ] Worker launch/model routing is deterministic. [x] Worktree cleanup is covered. | In progress | Spawn env baseline fixed; cleanup now removes safe clean worktrees even when unrelated backup blockers keep team state. Claude worker launch args now let explicit `--model` flags win and collapse duplicate/conflicting model flags to one canonical `--model <value>`. Next: audit terminal phase/model-routing gaps. |
