...
| persistent-mode | [x] Runtime hook cannot silently skip active nikoflow missing-engine enforcement. [x] Runtime/template missing-engine parity is checked. [x] Failure mode is explicit in tests. [ ] Live installed-hook smoke covers real hook config. | In progress | Runtime and installed template both fail closed when active nikoflow cannot load the TS engine. Next: live installed-hook smoke or broader Stop-hook parity audit. |
...
- Run live installed persistent-mode hook smoke after runtime/template missing-engine parity.
