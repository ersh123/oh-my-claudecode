...
function listTopLevelHookTemplates() {
...
function extractSettingsJqFilter(uninstallScript) {
...
describe('uninstall script .mjs hook parity', () => {
    ...
    it('removes OMC hook entries from settings without deleting third-party hooks', () => {
        ...
        const commands = Object.values(filtered.hooks ?? {}).flatMap((entries) => entries.flatMap((entry) => entry.hooks.map((hook) => hook.command)));
        expect(commands).toEqual(['node /opt/vendor/third-party.js']);
    });
});
//# sourceMappingURL=uninstall-mjs-parity.test.js.map