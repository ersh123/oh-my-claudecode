...
const PUBLIC_DOC_SCRIPT_EXPECTED_MARKERS = {
...
const STALE_PUBLIC_DOC_SCRIPT_TRIGGER_EXAMPLES = [
...
function runIsolatedKeywordDetector(prompt, scriptPath) {
...
describe('keyword-detector.mjs mode-message dispatch', () => {
    it('keeps standalone .mjs public trigger examples aligned with the TypeScript source of truth', () => {
        ...
        for (const [skillName, triggers] of Object.entries(KEYWORD_DETECTOR_PUBLIC_DOC_TRIGGER_EXAMPLES)) {
            for (const trigger of triggers) {
                for (const [label, scriptPath] of scriptPaths) {
                    const output = runIsolatedKeywordDetector(trigger, scriptPath);
                    const context = output.hookSpecificOutput?.additionalContext ?? '';
                    expect(context, `${label} should route "${trigger}" as ${skillName}`).toContain(PUBLIC_DOC_SCRIPT_EXPECTED_MARKERS[skillName]);
                }
            }
        }
    });
    it('does not keep stale public trigger examples alive in standalone .mjs hooks', () => {
        ...
        for (const { prompt, forbiddenMarker } of STALE_PUBLIC_DOC_SCRIPT_TRIGGER_EXAMPLES) {
            for (const [label, scriptPath] of scriptPaths) {
                ...
                const context = output.hookSpecificOutput?.additionalContext ?? '';
                expect(context, `${label} should not route stale trigger "${prompt}"`).not.toContain(forbiddenMarker);
            }
        }
    });
