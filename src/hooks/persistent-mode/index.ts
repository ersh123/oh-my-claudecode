...
/**
 * Per-Stop nikoflow state transaction (perf F1). ONE in-memory state object is
 * threaded through the whole Stop; every mutation happens via the pure `xxxIn`
 * cores on `ctx.state` and sets `dirty`; the orchestrator flushes exactly once
 * at the end of the Stop. HARD RULE: inside any ctx-threaded function, calls to
 * readNikoflowState/writeNikoflowState or the disk-variant mutators are
 * forbidden — a re-read returns the pre-rotation disk state and, if flushed,
 * would resurrect a rotated request-id (anti-self-approval, Fable QA R2). The
 * user-turn sidecar is a SEPARATE file and stays read-only here.
 */
type NikoflowCtx = {
  workingDir: string;
  sessionId: string | undefined;
  state: NikoflowState; // THE single in-memory copy for this Stop
  dirty: boolean;
};

function flushNikoflowCtx(ctx: NikoflowCtx): boolean {
...
function nikoflowExecuteError(
  ...
  return {
    shouldBlock: true,
    message:
      `<nikoflow-continuation phase="execute" iteration="${current.iteration}">\n` +
...
function nikoflowAdvanceFromExecute(ctx: NikoflowCtx): PersistentModeResult {
...
function nikoflowHardAbort(
  ...
  return {
    shouldBlock: true,
    message:
      `<nikoflow-blocked>NIKOFLOW ABORTED: ${ticketId} made no progress after ${stall} Stops ` +
 ...
 */
export function handleNikoflowExecute(
  workingDir: string,
  sessionId: string | undefined,
  current: NikoflowState,
  transcriptPath?: string,
  ...
  const ctx: NikoflowCtx = { workingDir, sessionId, state: current, dirty: false };
  const result = handleNikoflowExecuteCtx(ctx, transcriptPath);
  flushNikoflowCtx(ctx);
  return result;
}

function handleNikoflowExecuteCtx(
  ctx: NikoflowCtx,
  transcriptPath?: string,
): PersistentModeResult {
  const { workingDir, sessionId } = ctx;
  ...
  const preflightError = (error: string): PersistentModeResult => {
  ...
  if (ticket.status === 'review') {
    ...
    if (!reviewedSha) {
      ...
      return nikoflowExecuteError(
        current,
        `ticket ${ticket.id} has status "review" but no evidence.reviewed_sha — the hook always records ` +
    ...
    if (gitIsAncestorOfHead(workingDir, reviewedSha)) {
      ...
      if (tddErrors.length > 0) {
        ...
        return nikoflowExecuteError(
          current,
          `ticket ${ticket.id} is reviewer-approved and merged but evidence.tdd is missing/invalid: ` +
  ...
  const requestId: string | undefined = mintGateRequestIn(ctx.state, gate);
  ctx.dirty = true;

  // The ticket advances ONLY on a reviewer-subagent-authored TICKET_DONE tag.
  const approval: GateMatch =
  ...
  if (approval.matched) {
    ...
    if (tddErrors.length > 0) {
      ...
      return nikoflowExecuteError(
        current,
        `ticket ${ticket.id} got a reviewer approval but evidence.tdd is missing/invalid: ` +
...
function handleNikoflowVerifyCtx(
  ctx: NikoflowCtx,
  transcriptPath?: string,
): PersistentModeResult {
  const current = ctx.state;
  ...
  const requestId: string | undefined = mintGateRequestIn(ctx.state, 'verify');
  ctx.dirty = true;
...
export async function checkNikoflowLoop(
  ...
  const ctx: NikoflowCtx = { workingDir, sessionId, state, dirty: true };
  const current = ctx.state;
...
function nikoflowResumeHeaderIfFresh(
  workingDir: string,
  sessionId: string | undefined,
  preIncrementIteration: number,
  current: NikoflowState,
  transcriptPath?: string,
): string {
  if (preIncrementIteration <= 1) return '';
  ...
  if (tail.includes('<nikoflow-continuation')) return '';
  let tickets: NikoflowTicketsFile | null = null;
  try {
